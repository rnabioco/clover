...
and stores the results in a `SummarizedExperiment`. The main entry point is
`create_clover()`, which reads a pipeline `config.yaml` and loads counts,
base-calling error rates, and modification co-occurrence data.
