...
clover reads output from a tRNA sequencing pipeline and stores the results
in a `SummarizedExperiment`. The main entry point is `create_clover()`,
which reads a pipeline `config.yaml` and loads counts, base-calling error
rates, and modification co-occurrence data.

```{r}
#| label: ex-create-clover
#| eval: false
library(clover)
...
provides functions for plotting error profiles and heatmaps annotated with
Sprinzl structural coordinates.
...
within a single sample or changes between conditions (ratio of odds ratios).
