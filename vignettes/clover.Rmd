...
```{r}
#| label: bcerror-data
# Load preprocessed bcerror data (10 tRNAs: 5 most + 5 least abundant)
...
# Match tRNA names: strip "host-" prefix and convert DNA -> RNA anticodon
bcerror_delta <- bcerror_delta |>
  mutate(trna_id = dna_to_rna_anticodon(sub("^host-", "", ref)))

# Fetch known modification positions from MODOMICS
fasta_path <- S4Vectors::metadata(se)$config$fasta
mods <- modomics_mods(fasta_path, organism = "Escherichia coli")

# Join with sprinzl coords and modification annotations
bcerror_sprinzl <- bcerror_delta |>
  left_join(
    sprinzl |> select(trna_id, pos, sprinzl_label, global_index),
    by = c("trna_id", "pos")
  ) |>
  filter(!is.na(sprinzl_label)) |>
  left_join(
    mods |> distinct(ref, pos) |> mutate(has_mod = TRUE),
    by = c("ref", "pos")
  ) |>
  mutate(has_mod = tidyr::replace_na(has_mod, FALSE))

# Order positions by canonical Sprinzl sequence and clean ref names
label_order <- bcerror_sprinzl |>
  distinct(sprinzl_label, global_index) |>
  arrange(global_index) |>
  pull(sprinzl_label)

bcerror_sprinzl <- bcerror_sprinzl |>
  mutate(
    ref = sub("^host-", "", ref),
    sprinzl_label = factor(sprinzl_label, levels = label_order)
  )
